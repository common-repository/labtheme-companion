...
// register Labtheme_Contact_Social_Links widget 
function labtheme_register_contact_social_links_widget() {
    register_widget( 'Labtheme_Contact_Social_Links' );
}
add_action( 'widgets_init', 'labtheme_register_contact_social_links_widget' );


//load wp sortable
function labtheme_contact_load_sortable() {    
    wp_enqueue_script( 'jquery-ui-core' );    
    wp_enqueue_script( 'jquery-ui-sortable' );    
}
add_action( 'load-widgets.php', 'labtheme_contact_load_sortable' );

//allow skype
function labtheme_allowed_contact_social_protocols( $protocols ) {
    $social_protocols = array(
        'skype'
    );
    return array_merge( $protocols, $social_protocols );    
}
add_filter( 'kses_allowed_protocols' ,'labtheme_allowed_contact_social_protocols' );

 /**
 * Adds Labtheme_Contact_Social_Links widget.
 */
class Labtheme_Contact_Social_Links extends WP_Widget {
    ...
    function __construct() {
        ...
        parent::__construct(
            'labtheme_contact_social_links', // Base ID
            esc_html__( 'Lab Theme: Contact Widget', 'labtheme-companion' ), // Name
            array( 'description' => esc_html__( 'A Contact Widget', 'labtheme-companion' ), ) // Args
        );
    }
    ...
    */
    function labtheme_social_template() { ?>
        <div class="labtheme-contact-social-template">
            <li class="labtheme-contact-social-icon-wrap" data-id="{{ind}}">
                <span class="labtheme-contact-social-links-sortable-handle"></span>
                <span class="labtheme-contact-social-links-field-handle"><i class="fas fa-plus"></i></span>
                <label for="<?php echo esc_attr( $this->get_field_id( 'social_profile' ) ); ?>"><?php esc_html_e( 'Social Icon', 'labtheme-companion' ); ?></label>
                <span class="example-text">Example: facebook</span>
                <div class="social-search-wrap"><input class="user-contact-social-profile" placeholder="<?php _e('Search Social Icons','labtheme-companion');?>" id="" name="" type="text" value="" /></div>
                <label class="link-label" for="<?php echo esc_attr( $this->get_field_id( 'social' ) ); ?>"><?php esc_html_e( 'Link', 'labtheme-companion' ); ?></label>
                <span class="example-text">Example: http://facebook.com</span>
                <input class="labtheme-contact-social-length" name="" type="text" value="" />
                <span class="del-contact-labtheme-icon"><i class="fas fa-times"></i></span>
            </li>
        </div>
    <?php
    echo '<style>
        .labtheme-contact-social-template{
            display: none;
        }
        </style>';
    }
    ...
    public function widget( $args, $instance ) {
        ...
        <div class="labtheme-contact-widget-wrap contact-info">
        <?php 
            if($description!='') echo wpautop( wp_kses_post( $description ) ); 
            
            if( $phone || $email || $address ){ 
                echo '<ul class="contact-list">';
                if( $phone!='' ) echo '<li><i class="fa fa-phone"></i><b>'. esc_html__( 'Phone', 'labtheme-companion' ) . '</b><a href="' . esc_url( 'tel:' . preg_replace( '/\D/', '', $phone ) ) . '">' . esc_html( $phone ) . '</a></li>';
                if($email!='') echo '<li><i class="fa fa-envelope"></i><b>'. esc_html__( 'Email', 'labtheme-companion' ) . '</b><a href="' . esc_url( 'mailto:' . sanitize_email( $email ) ) . '">' . esc_html( $email ) . '</a></li>';
                if($address!='') echo '<li><i class="fa fa-map-marker"></i><b>'. esc_html__( 'Address', 'labtheme-companion' ) . '</b>' . esc_html( $address ) . '</li>';                
                echo '</ul>';
            }
                        ...
                        { 
                            ...
                            { 
                                if(isset($instance['social'][$value]) && !empty($instance['social'][$value]))
                                {
                                    ...
                                    ?>
                                    <li class="labtheme-contact-social-icon-wrap">
                                        <a <?php if(isset($instance['target']) && $instance['target']=='1'){ echo "target=_blank"; } ?> href="<?php echo esc_url($instance['social'][$value]);?>">
                                            <span class="labtheme-contact-social-links-field-handle"><i class="<?php echo esc_attr($class);?>"></i></span>
                                        </a>
                                    </li>
                                <?php
                                }
        ...
        <?php
        $html = ob_get_clean();
        echo apply_filters( 'labtheme_contact_widget_filter', $html, $args, $instance );
    ...
    public function form( $instance ) {
        ...
        <script type='text/javascript'>
            jQuery(document).ready(function($) {
                $('.labtheme-contact-sortable-links').sortable({
                    cursor: 'move',
                    update: function (event, ui) {
                        $('ul.labtheme-contact-sortable-links input').trigger('change');
                    }
                });
            });
        </script>
            <p>
                ...
                <input class="widefat labtheme-contact-social-title-test" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
            </p>

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description', 'labtheme-companion' ); ?></label> 
                <textarea class="widefat labtheme-contact-social-description-test" id="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'description' ) ); ?>"><?php echo esc_attr( $description ); ?></textarea>
            </p>

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'phone' ) ); ?>"><?php esc_html_e( 'Phone', 'labtheme-companion' ); ?></label> 
                <input class="widefat labtheme-contact-social-phone-test" id="<?php echo esc_attr( $this->get_field_id( 'phone' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'phone' ) ); ?>" type="tel" value="<?php echo esc_attr( $phone ); ?>" />
            </p>

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'email' ) ); ?>"><?php esc_html_e( 'Email', 'labtheme-companion' ); ?></label> 
                <input class="widefat labtheme-contact-social-email-test" id="<?php echo esc_attr( $this->get_field_id( 'email' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'email' ) ); ?>" type="email" value="<?php echo esc_attr( $email ); ?>" />
            </p>

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'address' ) ); ?>"><?php esc_html_e( 'Address', 'labtheme-companion' ); ?></label> 
                <input class="widefat labtheme-contact-social-address-test" id="<?php echo esc_attr( $this->get_field_id( 'address' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'address' ) ); ?>" type="text" value="<?php echo esc_attr( $address ); ?>" />
            </p>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>" class="check-btn-wrap"> 
                    ...
                    <?php esc_html_e( 'Open Social Links in New Tab', 'labtheme-companion' ); ?>
                </label>
            </p>

        <ul class="labtheme-contact-sortable-links" id="<?php echo esc_attr( $this->get_field_id( 'labtheme-contact-social-links' ) ); ?>">
        <?php
        if(isset($instance['social']) && !empty($instance['social']))
        {
            $icons  = $instance['social'];
                ...
                { 
                    ...
                    { 
                        if(isset($instance['social'][$value]) && !empty($instance['social'][$value]))
                        {
                            ...
                            ?>
                                <li class="labtheme-contact-social-icon-wrap" data-id="<?php echo $value;?>">
                                        <span class="labtheme-contact-social-links-sortable-handle"></span>
                                        <span class="labtheme-contact-social-links-field-handle"><i class="<?php echo esc_attr($class);?>"></i></span>
                                        ...
                                        <span class="example-text">Example: http://facebook.com</span>
                                        <input class="labtheme-contact-social-length" id="<?php echo esc_attr( $this->get_field_name( 'social['.$value.']' ) ) ?>" name="<?php echo esc_attr( $this->get_field_name( 'social['.$value.']' ) ) ?>" type="text" value="<?php echo esc_url($instance['social'][$value]);?>" />
                                        <span class="del-contact-labtheme-icon"><i class="fas fa-times"></i></span>
                                    </p>
                                </li>
                        <?php
        ...
        ?>
        <div class="labtheme-contact-social-icon-holder"></div>
        </ul>
        <input class="labtheme-contact-social-add button button-primary" type="button" value="<?php _e('Add Social Icon','labtheme-companion');?>"><br>
